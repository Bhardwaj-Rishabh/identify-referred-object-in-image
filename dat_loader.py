...
class ImgQuDataset(Dataset):
    ...
    def __init__(self, cfg, csv_file, ds_name, split_type='train', need_console=False):
        self.cfg = cfg
        ...
        self.need_console = need_console
        self.img_dir = Path(self.cfg.ds_info[self.ds_name]['img_dir'])
        self.image_data = self._read_annotations(csv_file, need_console=self.need_console)
    ...
    def _read_annotations(self, trn_file, need_console=False):
        #print(trn_file)
        ...
        #Added by Kritika
        if need_console:
            ipath = str(trn_data['img_id'].iloc[0])
            #print(self.img_dir / f'{ipath}')
            current_path_name = self.img_dir / f'{ipath}'
