...
def compute_avg(inp: List, nums: torch.tensor) -> float:
    ...
    #print("compute_avg-------------",inp, torch.stack(inp), nums.sum(), (torch.stack(inp) * nums).sum() / nums.sum())
    ...
    def update_prediction_file(self, predictions, pred_file):
        ...
        if self.cfg.do_dist:
            ...
            for p in predictions:
                pred_box.append(p['pred_boxes'])
                pred_scr.append(p['pred_scores'][0])    #Just storing the best pred score --> Kritika
            test_dat = pd.read_csv("data/referit/csv_dir/test.csv")
            test_dat['pred_box'] = pred_box
            test_dat['score'] = pred_scr
            test_dat.to_csv("true_vs_pred.csv")
